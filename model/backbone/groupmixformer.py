...
class EfficientAtt(nn.Module):
    def __init__(
        ...
        trans_dim = dim // 5 * 4
        self.crpe = ConvRelPosEnc(
            Ch=trans_dim // num_heads, h=num_heads, window={3: 2, 5: 3, 7: 3}
        )
...
class ConvStem(nn.Module):
    ...
    def __init__(self, in_dim=3, embedding_dims=64):
        ...
        self.proj2 = nn.Conv2d(
            mid_dim, embedding_dims, kernel_size=3, stride=2, padding=1
        )
...
class PatchEmbedLayer(nn.Module):
    def __init__(
        self, patch_size=16, in_dim=3, embedding_dims=768, is_first_layer=False
    ):
        super().__init__()
        if is_first_layer:
            patch_size = 1
...
def stochastic_depth(drop_path_rate, serial_depths, num_stages):
    ...
    dpr_per_stage = [
        dpr[sum(index_list[:i]) : sum(index_list[: i + 1])]
...
class GroupMixFormer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if pretrain is not None:
            ...
            if Nc1 != Nc2:
                print("Classifer weights not match")
                if Nc1 == 21841 and Nc2 == 1000:
                    ...
                    torch.nn.init.constant_(self.head.weight, 0.0)
    ...
    def reset_classifier(self, num_classes, global_pool=""):
        self.num_classes = num_classes
        self.head = (
            nn.Linear(self.embedding_dims[-1], num_classes)
            if num_classes > 0
            else nn.Identity()
        )
